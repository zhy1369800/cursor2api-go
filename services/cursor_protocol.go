...
func (s *CursorService) buildCursorRequest(request *models.ChatCompletionRequest) (cursorBuildResult, error) {
	...
	// 多模态增强：将消息中的图片也同步到顶层 Context 中，增强后端识别率
	contexts := make([]interface{}, 0)
	if len(cursorMessages) > 0 {
		// 重点扫描最后几条消息中的图片，模拟“当前对话上下文”
		for i := len(cursorMessages) - 1; i >= 0 && i >= len(cursorMessages)-2; i-- {
			msg := cursorMessages[i]
					...
					contexts = append(contexts, map[string]interface{}{
						"userItem": map[string]interface{}{
							"file": map[string]interface{}{
								"path":     fmt.Sprintf("attachment_%d.png", i),
								"contents": part.Image.Data,
							},
						},
					})
					// 注入为原生图片型 Context (核心)
	...
	for i, msg := range messages[startIdx:] {
		...
		if strings.EqualFold(msg.Role, "user") {
			...
			if text, ok := msg.Content.(string); ok {
				...
				msg.Content = text
			} else if parts, ok := msg.Content.([]models.ContentPart); ok {
				// 如果是多模态，找到第一个文本部分注入前缀，最后一个文本部分注入后缀
				textFound := false
				for j := range parts {
					if parts[j].Type == "text" {
						if !firstUserInjected {
							parts[j].Text = reframingPrefix + parts[j].Text
							firstUserInjected = true
						}
						textFound = true
						break
				...
				// 如果没找到文本部分（纯图片？），可以在开头插一个文本部分
				if !textFound && !firstUserInjected {
					parts = append([]models.ContentPart{{Type: "text", Text: reframingPrefix}}, parts...)
					firstUserInjected = true
				}
				...
				if i == lastUserIdx {
					for j := len(parts) - 1; j >= 0; j-- {
						if parts[j].Type == "text" {
							parts[j].Text = parts[j].Text + reframingSuffix
							suffixInjected = true
							break
						}
					}
					if !suffixInjected {
						parts = append(parts, models.ContentPart{Type: "text", Text: reframingSuffix})
					}
				}
				msg.Content = parts
			}
...
func convertMessage(msg models.Message, thinkingEnabled bool, triggerSignal string) (models.CursorMessage, bool) {
	...
	case "user":
		cursorMsg := models.CursorMessage{Role: "user"}

		// 1. 优先提取并添加图片部分 (Vision 模型通常更偏好图片放在文本前)
		if parts, ok := msg.Content.([]models.ContentPart); ok {
			for _, p := range parts {
				if p.Type == "image_url" && p.ImageURL != nil {
					mime, data, ok := models.ParseDataURL(p.ImageURL.URL)
					if ok {
						compressedData := models.CompressImage(data)
						finalMime := mime
						// 如果发生了压缩且原图不是 jpeg，则由于我们在 CompressImage 里统一输出成 jpeg，这里也要改 mime
						if compressedData != data {
							finalMime = "image/jpeg"
						}
						// 统一使用最高兼容性的 imageUrl 模式，同时保留 image 结构体作为冗余
						cursorMsg.Parts = append(cursorMsg.Parts, models.CursorPart{
							Type: "imageUrl",
							Image: &models.CursorImage{
								Data:     compressedData,
								MimeType: finalMime,
							},
							ImageUrl: fmt.Sprintf("data:%s;base64,%s", finalMime, compressedData),
						})
					}
				}
			}
		...
		if len(cursorMsg.Parts) == 0 {
			return models.CursorMessage{}, false
		}
		return cursorMsg, true
	default:
		text := msg.GetStringContent()
		cursorMsg := models.CursorMessage{Role: role}
		if text != "" {
			cursorMsg.Parts = append(cursorMsg.Parts, models.CursorPart{
				Type: "text",
				Text: text,
